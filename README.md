...
## 7. 结果（实测，RTX 3080 / bf16 / 60 epoch / 2 折）

### 7.1 GEV 关键点验证

| 配置 | KP F1@0.5 | KP F1@best | SROCC | MAE | SROCC ego | SROCC exo |
|---|---|---|---|---|---|---|
| crop10（不裁首尾） | **0.4885** | **0.4995** | 0.2706 | 0.889 | 0.250 | 0.299 |
| big（dim384/depth4） | **0.4885** | 0.4934 | 0.2471 | 0.897 | 0.216 | 0.276 |
| base（默认） | 0.4857 | 0.4915 | 0.2572 | 0.884 | 0.212 | 0.304 |
| no_worst（关掉最差帧通道） | 0.4793 | 0.4902 | 0.2397 | 0.968 | 0.210 | 0.264 |
| f64（64 帧） | 0.4495 | 0.4816 | **0.2721** | 0.918 | 0.246 | 0.296 |

**参照**：论文 GEVFormer = **0.5439**，论文 CLIP-GEV 朴素基线 = 0.4881，Random = 0.3178，
全预测多数类 = 0.3265。

### 7.2 ⚠️ 必须说清楚：这些配置差异在噪声里，不能排名

按折拆开看（同一配置的两个 fold）：

| 配置 | fold0 KP F1 | fold1 KP F1 | 差值 |
|---|---|---|---|
| base | 0.5094 | 0.4620 | **0.047** |
| big | 0.5417 | 0.4353 | **0.106** |
| crop10 | 0.5507 | 0.4264 | **0.124** |
| no_worst | 0.5200 | 0.4385 | 0.082 |
| f64 | 0.4792 | 0.4197 | 0.060 |

**折间方差（0.047~0.124）远大于配置间差异（最大 0.039）。** 结论：

- 「crop10 最好」这个说法**不成立**，2 折不足以区分这 5 个配置
- 唯一敢说的是：**关掉「最差帧」通道会略降**（0.4793 vs 0.4857），方向符合预期，但也在噪声内
- 要给出配置排名，每折至少需要独立的 3~5 折并报均值±标准差；当前 2 折不够

### 7.3 距离论文还差多少

**KP F1 0.4857~0.4995 vs 论文 0.5439，差 0.044~0.058。**
而且恰好落在论文的 **CLIP-GEV 朴素基线 0.4881** 上 —— 说明我们的时序建模
相对「冻结 CLIP 特征 + 线性层」几乎没带来增量。

**为什么没复现（按权重排序）**：

1. **架构不同**。论文是 2 层 Encoder(TCM) + 2 层 Decoder(CMV) + linear evaluator；
   我们是 3 层 encoder + attention pooling + 文本 cross-attention + 最差帧通道 + CORAL。
   这是两个不同的模型，本来就不该期待数字一样。
2. **多任务耦合**。论文的 GEVFormer 只训 GEV。我们同时训 score + keypoint + 动作分类 + 对齐，
   而且 `ScoreHead` 的输入**就是关键点概率** → 关键点头的梯度被分数任务污染。
   这是我为了「分数可解释」主动做的取舍，不是 bug，但代价确实体现在 KP F1 上。
3. **论文的 GEV 划分未公开**，只有 ego/exo 汇总 F1，没说哪些 record 进 train/test。
   我们用 GroupKFold(by record_id)。同模型换划分波动 ±0.03 很正常，所以这 0.05 的差距里
   有多少是真实差距、有多少是协议差异，**目前无法区分**。
4. **阈值**：best 阈值比固定 0.5 高约 0.01~0.02，论文大概率也调过。
5. **超参没正经搜过**。上面的配置差异都在噪声里，等于说搜索还没开始。

### 7.4 分数回归（论文没报这个口径，无法对标）

最好单折 SROCC = **0.3201**（base, fold0）。作为参考：
**只用真值关键点的「不达标比例」这一个特征，CV SROCC 就有 0.65**
（见附一节 B）。所以模型离「把关键点预测准」还差得远，而这是唯一的瓶颈。

### 7.5 尚未跑完的

上一轮并行扫参的调度器有个 `NameError` 只跑了第一波，以下 5 个配置（× 2 折）没跑：
`debias`（标注者去偏）、`noalign`（关掉跨视角对齐，λ=0）、`focal`（Focal Loss）、
`highdrop`（dropout 0.4）、`long150`（150 epoch）。它们的任务定义在 `/tmp/par3.py`。
